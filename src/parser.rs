...
impl Parser {
    ...
    fn parse_var_declaration(&mut self) -> Result<ASTNode, String> {
        ...
        Ok(ASTNode::new_var_declaration(
            name,
            type_hint,
            Box::new(value),
        ))
                        ...
                        if let Some(ref token) = self.current_token {
                            if token.token_type == TokenType::LParen {
                                ...
                                if let ASTNode::Identifier(func_name) = &result {
                                    ...
                                    self.expect(TokenType::RParen)?;
                                    result =
                                        ASTNode::new_function_call(func_name.clone(), arguments);
                                } else {
                                    return Err(
                                        "Cannot call function on non-identifier".to_string()
                                    );
                                }
                                ...
                                result =
                                    ASTNode::new_index_access(Box::new(result), Box::new(index));
                            } else {
                                break;
                            }
                        } else {
...
mod tests {
    ...
    fn test_parse_assignment() {
        let tokens = vec![identifier("x"), operator("="), number("10"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
    ...
    fn test_operator_precedence_division_over_subtraction() {
        ...
        match ast {
            ASTNode::Program(stmts) => match stmts[0].as_ref() {
                ASTNode::BinaryOp {
                    left,
                    operator: op,
                    right,
    ...
    fn test_parse_unary_minus() {
        let tokens = vec![operator("-"), number("5"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
    ...
    fn test_parse_unary_not() {
        let tokens = vec![keyword("hoina"), keyword("sahi"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
    ...
    fn test_parse_print_statement() {
        let tokens = vec![keyword("bhan"), string("hello"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
    ...
    fn test_parse_return_statement() {
        let tokens = vec![keyword("pathau"), number("42"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();

        match ast {
            ASTNode::Program(stmts) => match stmts[0].as_ref() {
                ASTNode::Return(value) => match value.as_ref() {
                    ASTNode::Number(n) => assert_eq!(n, "42"),
                    _ => panic!("Expected number"),
                },
    ...
    fn test_parse_break_statement() {
        let tokens = vec![keyword("rok"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
    ...
    fn test_parse_continue_statement() {
        let tokens = vec![keyword("jane"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
    ...
    fn test_parse_import_statement() {
        let tokens = vec![keyword("aayaat"), string("module.nep"), eof()];
        let mut parser = Parser::new(tokens);
        let ast = parser.parse().unwrap();
    ...
    fn test_parse_comparison_operators() {
        ...
        for op in operators {
            let tokens = vec![number("5"), operator(op), number("10"), eof()];
            let mut parser = Parser::new(tokens);
            let ast = parser.parse().unwrap();
    ...
    fn test_parse_all_arithmetic_operators() {
        ...
        for op in operators {
            let tokens = vec![number("10"), operator(op), number("5"), eof()];
            let mut parser = Parser::new(tokens);
            let ast = parser.parse().unwrap();
    ...
    fn test_parse_error_missing_closing_paren() {
        let tokens = vec![make_token(TokenType::LParen, "("), number("5"), eof()];
        let mut parser = Parser::new(tokens);
        let result = parser.parse();
